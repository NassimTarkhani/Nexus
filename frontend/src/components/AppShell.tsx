...
type AppView = "chat" | "mcp" | "workflow" | "archive" | "settings";

interface AppShellProps {
    initialView?: AppView;
}

export default function AppShell({ initialView = "chat" }: AppShellProps) {
    ...
    // Fetch on auth ready + re-fetch whenever view switches to chat
    useEffect(() => {
        if (!initialized || !user?.id) return;
        refreshConversations();
    }, [initialized, user?.id, activeView, refreshConversations]);
                                ...
                                {conversations.length > 0 ? (
                                    conversations.slice(0, 20).map((conv) => (
                                        <div
                                            key={conv.id}
                                            className={cn(
                                                "group w-full flex items-center gap-2 px-3 py-2 rounded-lg transition-all text-sm text-left cursor-pointer",
                                        ...
                                        >
                                            ...
                                            <button
                                                ...
                                                className="opacity-0 group-hover:opacity-100 p-0.5 hover:text-red-400 transition-all rounded flex-shrink-0"
